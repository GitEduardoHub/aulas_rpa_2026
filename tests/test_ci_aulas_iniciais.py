...
def flake8_steps(steps):
    ...
    return [
        ...
        for step in steps
        if "run" in step
        and any(line.strip().startswith("flake8") for line in step["run"].splitlines())
    ]
...
class TestFlake8ExcludeVenv:
    ...
    def test_critical_syntax_step_excludes_venv(self, flake8_steps):
        ...
        critical_steps = [
            s
            for s in flake8_steps
            if "E9,F63,F7,F82" in s["run"] or "select=E9" in s["run"]
        ]
        assert critical_steps, "Step de erros críticos do flake8 não encontrado"
        step = critical_steps[0]
        assert (
            "--exclude=.venv" in step["run"]
    ...
    def test_pep8_step_excludes_venv(self, flake8_steps):
        ...
        pep8_steps = [s for s in flake8_steps if "max-line-length" in s["run"]]
...
class TestCacheStep:
    ...
    def test_cache_uses_v4(self, cache_step):
        ...
        assert (
            cache_step["uses"] == "actions/cache@v4"
    ...
    def test_cache_key_contains_hash_files(self, cache_step):
        ...
        key = cache_step["with"]["key"]
        assert "hashFiles(" in key, f"Chave de cache não contém 'hashFiles(...)': {key}"
        assert (
            "requirements.txt" in key
    ...
    def test_cache_key_full_structure(self, cache_step):
        ...
        assert all(
            component in key
            for component in ["runner.os", "matrix.python-version", "hashFiles"]
